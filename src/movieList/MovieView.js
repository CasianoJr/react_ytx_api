...
export default function MovieView({ movie }) {
  const imageFallback =
    "https://www.leicestercathedral.org/wp-content/uploads/2019/03/abstract-astrology-astronomy-1376766-350x525.jpg";
  ...
  return (
    <div className="col-lg-2 col-md-3 col-sm-4 col-8 mx-lg-3 mx-auto p-0">
      <Link to={movie.id.toString()}>
        <div className="text-center mx-sm-3">
          <Image
            ...
            className="w-100 px-sm-2 px-md-0 px-3"
      ...
      <div>
        <div className="text-muted text-center justify-content-between d-flex mx-lg-3 mx-md-3 mx-sm-4  mx-2 ">
          <span>{movie.year}</span>
          <span className="small">{movie.runtime}m</span>

          <span>
            {movie.torrents.map((torrentSite) => (
              <a
                className="mx-1"
                key={torrentSite.hash}
                href={`magnet:?xt=urn:btih:${torrentSite.hash}`}
                alt="movie"
                onClick={(e) => {
                  e.preventDefault();
                  window.open(`magnet:?xt=urn:btih:${torrentSite.hash}`);
                }}
              >
