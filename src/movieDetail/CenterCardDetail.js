                ...
                <div className="row mt-5 shadow-lg">
                    {movie.torrents.map((torrent, idx) =>
                        <div key={idx} className="col-sm-4 text-center mx-auto">
                            ...
                            <div>
                                <a className="btn btn-success px-1" href={`magnet:?xt=urn:btih:${torrent.hash}`} onClick={(e) => { e.preventDefault(); window.open(`magnet:?xt=urn:btih:${torrent.hash}`) }}  >
                                    <img style={{ height: 15, width: 15 }} src={magnetTorrent} alt={movie.title} />
                                Magnet
                            </a>
                            </div>
                            <div>
                                <a className="btn btn-success my-1" href={torrent.url}>Torrent</a>
                            </div>
                        </div>
